...
4. ~~决定是否引入 SFT 冷启动~~ ✅ **已引入并验证**(§5.1):救回 7/14。项目定位从「纯 RL-zero」
   调整为 **SFT 冷启动 → GDPO RL**。
5. ~~下一步:从 SFT 合并 ckpt 起步,跑 SFT→GDPO 完整训练~~ ✅ 已跑,但曲线持平 —— 见 §7。

---

## 7. 后续(2026-06-07):曲线持平的真因 = user-sim,不是模型/GDPO

按 §6.5 跑了 SFT→GDPO,**训练曲线持平**(reward 卡 ~0.48、grad_norm ~0.02)。沿用本文方法继续排查后
发现:**根因不在模型,也不在 GDPO,而在 06-06 把 user-sim 从 gpt-5 换成了本地 `Qwen3.6-35B-A3B`。**
这个 3B-active 小模型守不住「顾客」角色,78% 的对话翻转成客服并逐字复读死循环,把 reward 信号整体压掉
约 90% —— 本文 §5.1 里被 SFT(用 gpt-5)救活的 18/24/29/39/42 在新 user-sim 下又全 0(出生即死)。

换成 **DeepSeek V4(flash)** 后角色翻转降到 1%、信号恢复到 gpt-5 的 ~81%,重启训练后 db std ~0.49
(组内有方差、有梯度)。完整排查、受控 A/B、机制(角色翻转/镜像死循环)与落地改动见
**`docs/usersim_rootcause_deepseek_fix.md`**。

> 修正本文的部分归因:§2 病因③把若干任务判为「模型能力不会」是在 **gpt-5 user-sim** 下成立的;
> 在本地 A3B user-sim 下,这些任务的失败其实是 **user-sim 把对话带歪**,与模型能力无关。
